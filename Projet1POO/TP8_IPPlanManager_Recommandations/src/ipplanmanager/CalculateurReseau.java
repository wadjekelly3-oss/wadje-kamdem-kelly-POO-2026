...
public class CalculateurReseau {
    ...
    public static String obtenirMasqueDecimal(int cidr) {
        ...
        int o1 = (masque >>> 24) & 255, o2 = (masque >>> 16) & 255;
        int o3 = (masque >>> 8) & 255, o4 = masque & 255;
        return o1 + "." + o2 + "." + o3 + "." + o4;
    }
    
    public static int convertirIPEnEntier(String ip) {
        String[] p = ip.split("\\."); int r = 0;
        for (int i = 0; i < 4; i++) r = r * 256 + Integer.parseInt(p[i]);
        return r;
    }
    ...
    public static String calculerPremiereAdresseUtilisable(String a) {
        return convertirEntierEnIp(convertirIPEnEntier(a) + 1);
    }
    
    public static String calculerDerniereAdresseUtilisable(String a, int c) {
        return convertirEntierEnIp(convertirIPEnEntier(a) + calculerTailleBloc(c) - 2);
    }
    ...
    public static boolean estAdresseIPValide(String ip) {
        ...
        for (String s : p) {
            try { int v = Integer.parseInt(s); if (v < 0 || v > 255) return false; }
    ...
    public static void verifierAdresseIP(String ip) throws AdresseIPInvalideException {
        if (!estAdresseIPValide(ip)) throw new AdresseIPInvalideException("Adresse IP invalide : " + ip);
    }
    ...
    public static boolean reseauxSeChevauchent(String a1, int c1, String a2, int c2) {
        int d1 = convertirIPEnEntier(a1), f1 = calculerAdresseFin(a1, c1);
        int d2 = convertirIPEnEntier(a2), f2 = calculerAdresseFin(a2, c2);
        return d1 <= f2 && d2 <= f1;
    }
}